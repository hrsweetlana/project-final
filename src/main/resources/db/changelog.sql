...
(
    ...
    constraint FK_PROFILE_USERS foreign key (ID) references APP_USERS (ID) on delete cascade
);
...
--     history of task field change
    ...
    constraint FK_ACTIVITY_USERS foreign key (AUTHOR_ID) references APP_USERS (ID),
...
(
    ...
    STARTPOINT     timestamp,
    ENDPOINT       timestamp,
    constraint FK_USER_BELONG foreign key (APP_USER_ID) references APP_USERS (ID)
);
create unique index UK_USER_BELONG on USER_BELONG (OBJECT_ID, OBJECT_TYPE, APP_USER_ID, USER_TYPE_CODE);
create index IX_USER_BELONG_USER_ID on USER_BELONG (APP_USER_ID);
...
(
    ...
    constraint UK_USER_ROLE unique (APP_USER_ID, APP_ROLES),
    constraint FK_USER_ROLE foreign key (APP_USER_ID) references APP_USERS (ID) on delete cascade
);
...
alter table ACTIVITY
    add constraint FK_ACTIVITY_USERS foreign key (AUTHOR_ID) references APP_USERS (ID) on delete cascade;

alter table USER_BELONG
    drop constraint FK_USER_BELONG;
alter table USER_BELONG
    add constraint FK_USER_BELONG foreign key (APP_USER_ID) references APP_USERS (ID) on delete cascade;

alter table ATTACHMENT
    drop constraint FK_ATTACHMENT;
alter table ATTACHMENT
    add constraint FK_ATTACHMENT foreign key (APP_USER_ID) references APP_USERS (ID) on delete cascade;
